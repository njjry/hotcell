...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val resultsDF = spark.sql("SELECT rectangle, COUNT(rectangle) AS value FROM joinResult GROUP BY rectangle ORDER BY rectangle")
    //results.collect().foreach(println)
    // YOU NEED TO CHANGE THIS PART

    return resultsDF // YOU NEED TO CHANGE THIS PART
  }
